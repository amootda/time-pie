      ...
      <HoverCard.Root openDelay={0} closeDelay={100}>
        ...
        >
          ...
          {slices.map((slice, index) => {
            ...
            return (
              <HoverCard.Root
                key={`${slice.event?.id || index}-${slice.startAngle}`}
                openDelay={0}
                closeDelay={50}
              >
                ...
                {!slice.isEmpty && slice.event && (
                  <HoverCard.Portal>
                    <HoverCard.Content
                      ...
                      align="center"
                      sideOffset={5}
                      className="border-border bg-popover/95 text-popover-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 z-50 w-64 rounded-xl border p-4 shadow-lg outline-none backdrop-blur-sm"
                    >
                      <div className="flex flex-col space-y-2">
                        ...
                        <p className="text-muted-foreground mt-1 text-sm">
                          ...
                          {new Date(slice.event.start_at).toLocaleTimeString(
                            [],
                            {
                              hour: '2-digit',
                              minute: '2-digit',
                          ...
                          -{' '}
                          {new Date(slice.event.end_at).toLocaleTimeString([], {
                            hour: '2-digit',
                            minute: '2-digit',
                            hour12: false,
                          })}
            ...
            <g>
              ...
              {(() => {
                const currentSlice = slices.find((slice) => {
                  ...
                  return (
                    (now.isAfter(startTime) || now.isSame(startTime)) &&
                ...
                return currentSlice?.event ? (
                  <text
                    ...
                    y={center + 25}
                    textAnchor="middle"
                    className="fill-cyan-400 text-[11px] font-semibold uppercase tracking-wide"
                  >
          ...
          {showLabels &&
            HOUR_LABELS.map((hour, i) => {
              const pos = getHourLabelPosition(
                hour,
                center,
                center,
                labelRadius
              ...
              return (
                <text
                  ...
                  className="fill-muted-foreground pointer-events-none text-xs font-medium"
                >
